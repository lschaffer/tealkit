...
import 'package:tealkit_server/runner/server_embedded_types.dart';
...
/// Injected database adapter — set by [serverBootstrap].
/// Uses [defaultDbFactory] from server_database_adapter.dart to avoid
/// directly importing server_duckdb_adapter.dart (and transitively dart_duckdb).
ServerDatabaseAdapter _db = defaultDbFactory();

/// Set by the full-server entry point to [ServerEmbeddedLlmAdapter.instance].
/// server_light does NOT set this — embedded LLM endpoints will fail at
/// runtime (correct for light mode). Uses [dynamic] to avoid importing
/// server_embedded_llm_adapter.dart (and transitively llamadart).
dynamic embeddedLlmInstance;

/// Feature flags controlling which routes/services are active.
ServerFeatureFlags _flags = const ServerFeatureFlags();
...
final String _npmGlobalPrefix = () {
  if (Platform.isWindows) {
    final home =
        Platform.environment['USERPROFILE'] ??
        Platform.environment['APPDATA'] ??
        'C:\\';
    return p.join(home, '.tealkit-server', 'npm-global');
  }
  return '/data/.npm-global';
}();
...
Map<String, String> _serverRuntimeEnv([Map<String, String>? overrides]) {
  ...
  if (Platform.isWindows) {
    env['NPM_CONFIG_CACHE'] =
        env['NPM_CONFIG_CACHE'] ??
        p.join(Platform.environment['LOCALAPPDATA'] ?? 'C:\\', 'npm-cache');
  } else {
  ...
  env['PATH'] = currentPath.isEmpty
      ? _npmGlobalPrefix
      : '$_npmGlobalPrefix$pathSep$currentPath';
...
Future<void> _bootstrapBundledMcpServers(ServerDatabaseAdapter db) async {
...
///
/// Accepts optional [db] and [flags] parameters for server_light reuse.
Future<void> serverBootstrap({
  ServerDatabaseAdapter? db,
  ServerFeatureFlags? flags,
}) async {
  _db = db ?? defaultDbFactory();
  serverDb = _db;
  _flags = flags ?? const ServerFeatureFlags();
  final dataDir = _db.dataDir;
  ...
  await _bootstrapBundledMcpServers(_db);
  ...
  if (_flags.enableEmbeddedModels) {
    await ServerModelDownloadService.instance.init();
  }

  // 5. Scheduler
  if (_flags.enableScheduler) {
    ServerScheduler.instance.start();
  }

  // 6. Start output cleanup timer (every 15 minutes)
  _startOutputCleanupTimer(dataDir);
  ...
  ProcessSignal.sigint.watch().listen((_) async {
    ...
    _stopOutputCleanupTimer();
    if (_flags.enableScheduler) ServerScheduler.instance.stop();
    exit(0);
  });
  if (!Platform.isWindows) {
    try {
...
Timer? _outputCleanupTimer;

void _startOutputCleanupTimer(String dataDir) {
  _outputCleanupTimer?.cancel();
  log.info(
    '[Cleanup] Starting output cleanup timer — every 30 minutes, root: $dataDir/output',
  );
  _outputCleanupTimer = Timer.periodic(const Duration(minutes: 30), (_) {
    _cleanupOutputFiles(dataDir);
  });
  // Also run once immediately at startup
  Timer.run(() => _cleanupOutputFiles(dataDir));
}

void _stopOutputCleanupTimer() {
  _outputCleanupTimer?.cancel();
  _outputCleanupTimer = null;
  log.info('[Cleanup] Output cleanup timer stopped');
}

Future<void> _cleanupOutputFiles(String dataDir) async {
  try {
    final retentionDays = ServerPreferencesService.instance.outputRetentionDays;
    if (retentionDays <= 0) {
      log.info('[Cleanup] Retention days is 0 — skipping cleanup');
      return;
    }

    final outputDir = Directory('$dataDir/output');
    if (!await outputDir.exists()) {
      log.info('[Cleanup] Output directory does not exist: ${outputDir.path}');
      return;
    }

    final cutoff = DateTime.now().subtract(Duration(days: retentionDays));
    log.info(
      '[Cleanup] Scanning ${outputDir.path} for files older than $retentionDays day(s) (before ${cutoff.toIso8601String()})',
    );
    ...
    await for (final taskDir in outputDir.list()) {
      if (taskDir is! Directory) continue;
      scannedTasks++;
      final taskName = taskDir.uri.pathSegments.last;

      // Scan run directories inside each task
      await for (final runDir in taskDir.list()) {
        if (runDir is! Directory) continue;
        scannedRuns++;
        final runName = runDir.uri.pathSegments.last;

        // Check each file in the run directory
        var runHasRecentFiles = false;
        await for (final file in runDir.list()) {
          if (file is! File) continue;
          final stat = await file.stat();
          if (stat.modified.isAfter(cutoff)) {
            runHasRecentFiles = true;
            break;
          }
        }

        // Delete files older than cutoff, then delete dir if empty
        if (!runHasRecentFiles) {
          log.info('[Cleanup] Purging old run: $taskName/$runName');
          await for (final file in runDir.list()) {
            if (file is File) {
              await file.delete();
              deletedFiles++;
          ...
          final remaining = await runDir.list().toList();
      ...
      final remaining = await taskDir.list().toList();
      if (remaining.isEmpty) {
        log.info('[Cleanup] Removing empty task directory: $taskName');
        await taskDir.delete();
        deletedDirs++;
      }
    }

    log.info(
      '[Cleanup] Complete — scanned $scannedTasks task(s), $scannedRuns run(s), '
      'removed $deletedFiles file(s) and $deletedDirs dir(s) older than $retentionDays day(s)',
    );
  } catch (e, st) {
    log.warning('[Cleanup] Error: $e', e, st);
  }
}
...
Future<void> startHttpServer({
  ...
  router.get(
    '/api/v1/mcp/servers/<serverId|.+>/events',
    _handleMcpServerEvents,
  );
...
Future<Response> _handleHealth(Request request) async {
  return _json({
    ...
    'mode': _flags.enableEmbeddedModels ? 'full' : 'light',
    'engine': _db.engineType,
  });
}
...
Future<Response> _handleGetTask(Request request, String id) async {
  try {
    final task = await _db.getTask(id);
...
Map<String, dynamic> _taskTransportJson(AgenticTask task) {
  ...
  final isRunning =
      ServerScheduler.instance.isTaskRunning(task.id) ||
...
Future<Response> _handleListModelFiles(Request request) async {
  try {
    if (!_flags.enableEmbeddedModels) {
      return _jsonError(
        'Embedded models not available in light server mode',
        status: 503,
      );
    }
...
Future<Response> _handleGetLoadedModel(Request request) async {
  try {
    ...
    final adapter = embeddedLlmInstance;
...
Future<Response> _handleUnloadModel(Request request) async {
  try {
    ...
    await embeddedLlmInstance.dispose();
...
Future<Response> _handleLlmChatCompletions(Request request) async {
  ...
  final headerProvider =
      request.headers['x-llm-provider'] ?? request.headers['X-Llm-Provider'];
  if (headerProvider != null && headerProvider.isNotEmpty) {
    ...
    if (modelLower.contains('gemini')) {
      ...
      resolvedProvider = LlmProvider.openai;
        ...
        modelLower.startsWith('open-mixtral')) {
  ...
  if (resolvedProvider == LlmProvider.embedded || requestedModelLooksEmbedded) {
    ...
    if (requestedModelLooksEmbedded &&
        resolvedProvider != LlmProvider.embedded) {
      log.info(
  ...
  final headerBaseUrl =
      request.headers['x-llm-base-url'] ?? request.headers['X-Llm-Base-Url'];
  final headerApiKey =
      request.headers['x-llm-api-key'] ?? request.headers['X-Llm-Api-Key'];

  switch (resolvedProvider) {
    case LlmProvider.openai:
      upstreamBase = 'https://api.openai.com/v1';
      upstreamAuth =
          headerApiKey ?? llm.getApiKeyForProvider(LlmProvider.openai);
    case LlmProvider.mistral:
      upstreamBase = 'https://api.mistral.ai/v1';
      upstreamAuth =
          headerApiKey ?? llm.getApiKeyForProvider(LlmProvider.mistral);
    case LlmProvider.gemini:
      ...
      upstreamAuth =
          headerApiKey ?? llm.getApiKeyForProvider(LlmProvider.gemini);
    case LlmProvider.ollama:
      final base =
          (headerBaseUrl ?? llm.getBaseUrlForProvider(LlmProvider.ollama));
      final cleanBase = (base.isNotEmpty ? base : 'http://localhost:11434')
          .trimRight()
          .replaceAll(RegExp(r'/+$'), '');
      final sanitizedBase = cleanBase.endsWith('/api')
          ? cleanBase.substring(0, cleanBase.length - 4)
          : cleanBase;
      upstreamBase = '$sanitizedBase/v1';
      upstreamAuth =
          headerApiKey ??
          (llm.getApiKeyForProvider(LlmProvider.ollama).isNotEmpty
              ? llm.getApiKeyForProvider(LlmProvider.ollama)
              : null);
    case LlmProvider.openaiCompatible:
      final base =
          (headerBaseUrl ??
          llm.getBaseUrlForProvider(LlmProvider.openaiCompatible));
      upstreamBase = base.trimRight().replaceAll(RegExp(r'/+$'), '');
      upstreamAuth =
          headerApiKey ??
          (llm.getApiKeyForProvider(LlmProvider.openaiCompatible).isNotEmpty
              ? llm.getApiKeyForProvider(LlmProvider.openaiCompatible)
              : null);
    default:
      return _jsonError(
...
Future<Response> _handleLlm2ChatCompletions(Request request) async {
  ...
  switch (resolvedProvider) {
    ...
    case LlmProvider.ollama:
      ...
      upstreamAuth =
          headerApiKey ?? (llm.apiKey2.isNotEmpty ? llm.apiKey2 : null);
    case LlmProvider.openaiCompatible:
      final base = (headerBaseUrl ?? llm.baseUrl2).trimRight().replaceAll(
        RegExp(r'/+$'),
        '',
      );
      upstreamBase = base;
...
Future<Response> _handleSyncPyTools(Request request) async {
  ...
  try {
    if (syncedIds.isEmpty) {
      await _db.execute('DELETE FROM py_tools');
    } else {
      final escapedIds = syncedIds.map((id) => "'${_esc(id)}'").join(', ');
      await _db.execute('DELETE FROM py_tools WHERE id NOT IN ($escapedIds)');
    }
  } catch (e) {
...
Future<Response> _handleSyncJsTools(Request request) async {
  ...
  try {
    if (syncedIds.isEmpty) {
      await _db.execute('DELETE FROM js_tools');
    } else {
      final escapedIds = syncedIds.map((id) => "'${_esc(id)}'").join(', ');
      await _db.execute('DELETE FROM js_tools WHERE id NOT IN ($escapedIds)');
    }
  } catch (e) {
    errors.add('delete_stale: $e');
  }
...
Future<Response> _handleStartMcpServer(Request request, String serverId) async {
  ...
  if (internalTypes.contains(normServerId)) {
    try {
      ...
      final impl = await createServerInternalMcp(
        normServerId,
        effectiveInitParams,
      );
      if (impl == null) {
...
Future<Response> _startExternalMcpServer(McpToolConfig cfg) async {
  ...
  final resolvedUrl = parsedResolved.path.toLowerCase().endsWith('/mcp')
      ? resolvedBase
      : parsedResolved.replace(path: parsedResolved.path + endpoint).toString();
...
Future<Response> _handleCallMcpTool(
  ...
  if (internalImpl != null) {
    try {
      ...
      log.error(
        '[API] Internal MCP tool call $normServerId/$toolName failed: $e',
      );
...
Future<Response> _handleListPlaygroundSessions(Request request) async {
  try {
    final sessions = await _db.getAllPlaygroundSessions();
...
Future<Response> _handleSavePlaygroundSession(Request request) async {
  ...
  try {
    await _db.savePlaygroundSession(body);
...
Future<Response> _handleDeletePlaygroundSession(
  ...
  try {
    await _db.deletePlaygroundSession(id);
    return _json({'deleted': id});
  } catch (e) {
