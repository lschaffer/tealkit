...
/// Server-side DuckDB service.
///
/// Equivalent to the Flutter app's [DuckDbService] but resolves its data
/// directory from the `TEALKIT_DATA_DIR` environment variable (falling back
/// to `~/.tealkit-server`).  No path_provider dependency.
class ServerDuckDbService {
  static const String _dbFileName = 'tealkit_server.duckdb';

  Database? _db;
  Connection? _conn;

  static final ServerDuckDbService _instance = ServerDuckDbService._internal();
  factory ServerDuckDbService() => _instance;
  ServerDuckDbService._internal();

  // ──────────────────────────────────────────────
  // Data directory — resolved once at startup
  // ──────────────────────────────────────────────
  ...
  Future<void> _createTables() async {
    ...
    try {
      ...
      // no-op for older DuckDB engines without ADD COLUMN IF NOT EXISTS
    }
    ...
    try {
      ...
      // no-op for engines without IF NOT EXISTS support
    }
    ...
    // Scheduler run log — server-specific, not in GUI app
    await conn.execute('''
  ...
  Future<AgenticTask?> getTask(String id) async {
    ...
    final dataRaw = row[0];
    return AgenticTask.fromJson(_decodeJsonMap(dataRaw));
  }
  ...
  Future<List<AgenticTask>> getDueTasks() async {
    final conn = await connection;
    // Query all enabled tasks — due-ness is computed from cron + last_run
    // rather than a stored next_run field.  This ensures tasks pushed from
    // the app with next_run=null (e.g. never run locally, or rescheduled)
    // still fire correctly on the server.
    final rows = (await conn.query(
    ...
    return rows
        ...
        .where((task) {
          ...
          // Compute the first scheduled fire after the last run (or epoch if
          // never run).  If that fire time is now or in the past, the task
          // is due.  Using epoch as the reference when there is no lastRun
          // means "has this cron slot ever been missed?" which correctly
          // fires tasks that were pushed to the server but never executed.
  ...
  Future<List<InternalMcpEntry>> getInternalMcpConfigs(String taskId) async {
    ...
    return rows.map((row) {
      final params = _decodeJsonMap(row[2]);
  ...
  /// Seed built-in default Python tools when the `py_tools` table is empty.
  ...
  /// Returns the 3 built-in default Python tool definitions as maps.
  List<Map<String, dynamic>> _defaultPyTools(String now) {
    return [
      // ── csv_analyzer ────────────────────────────────────────────────
      {
        ...
        'name': 'csv_analyzer',
        'description':
            'Parse and profile CSV data: column-level statistics, '
            'null counts, frequencies, quartiles, and type detection.',
        'input_schema': {
          'type': 'object',
          'properties': {
            'csv_data': {
      ...
      // ── json_query ──────────────────────────────────────────────────
      {
        ...
        'name': 'json_query',
        'description':
            'Filter, project, sort, group-by, and aggregate JSON datasets '
            'using simple Python expressions.',
        'input_schema': {
          'type': 'object',
          'properties': {
            'data': {
      ...
      // ── text_classify ───────────────────────────────────────────────
      {
        ...
        'description':
            ...
            'Returns best-matching labels with confidence scores.',
        'input_schema': {
          'type': 'object',
          'properties': {
            'text': {
  ...
  Future<void> _migrateTasksToOrchestratorPattern() async {
    ...
    try {
      ...
      for (final id in allTasks.keys) {
        ...
        // Phase 1 migration: ensure it has executors
        bool modified = false;
...
// ═════════════════════════════════════════════════════════════════════════════
// Default Python tool code constants (stdlib-only, no external deps)
// ═════════════════════════════════════════════════════════════════════════════
