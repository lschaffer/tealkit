...
///
/// This implementation intentionally avoids external ML runtime dependencies.
/// It builds a deterministic normalized vector by hashing tokens and character
...
class SemanticEmbedding {
  ...
  /// Build a deterministic normalized embedding vector.
  static List<double> buildEmbedding(String text, {int dimensions = defaultDimensions}) {
    ...
    for (final entry in tf.entries) {
      ...
      final trigrams = _charTrigrams(token);
  ...
  static double keywordOverlapScore(String query, String document) {
    ...
    final matches = q.where(d.contains).length;
