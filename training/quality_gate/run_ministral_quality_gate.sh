...
#
# Quality gate for Ministral 3B trained via Colab (Unsloth, Mistral [INST] template).
# Use this AFTER Colab Cell 10 has pushed the new model to HF.
#
# Usage:
#   # Download freshly trained model from HF and test:
#   ./run_ministral_quality_gate.sh --from-hf
#
#   # Provide a local GGUF (e.g. downloaded from Google Drive):
#   ./run_ministral_quality_gate.sh --gguf /path/to/ministral-3b-weathersensorsmcp-unsloth-Q4_K_M.gguf
#
# Options:
#   --gguf PATH          Use a local GGUF file (skips HF download)
#   --from-hf            Download from HF (re-downloads if already cached)
#   --hf-repo REPO       HF repo to pull from (default: lschaffer/ministral-3b-weathersensorsmcp)
#   --filename NAME      GGUF filename in the repo (default: ministral-3b-weathersensorsmcp-unsloth-Q4_K_M.gguf)
#   --model-name NAME    Ollama model name to register (default: ministral-3b-weathersensorsmcp)
...
# Defaults
HF_REPO="lschaffer/ministral-3b-weathersensorsmcp"
GGUF_FILENAME="ministral-3b-weathersensorsmcp-unsloth-Q4_K_M.gguf"
OLLAMA_MODEL_NAME="ministral-3b-weathersensorsmcp"
LOCAL_GGUF_PATH=""
FROM_HF=0
NUM_CTX=16384
WORK_DIR="$SCRIPT_DIR/../weathersensorsmcp/ministral_3b_gate_tmp"
