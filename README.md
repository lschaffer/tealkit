...
* **TealKit CLI v1.1.0 & Autonomous Coding Agent (`tealkit code`)** — Interactive Claude Code / Roo Code style terminal coding agent with **Architect (`/plan`)**, **Code (`/code`)**, and **Ask (`/ask`)** modes. Supports multi-turn codebase exploration, structured **`tasks.md`** planning, custom **`tealkit_agent.md`** instructions, and native pure-Dart tools (`fs_find`, `fs_read_file`, `fs_write_file`, `fs_replace_text`, `terminal_exec`) now shared across CLI, Desktop, and Server through `dart_mcp_core`. See the [TealKit CLI Guide](cli/README.md).
